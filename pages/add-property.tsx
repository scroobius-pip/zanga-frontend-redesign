...
const Page = ({ user, token }: Props) => {
    ...
    useEffect(() => {
        const isValid = title?.length <= 65 && title?.length >= 1 && !(!price || !(parseFloat(price) > 0)) && city?.length >= 1 && description?.length >= 1 && imagesFiles?.length >= 1
        setValid(isValid)
    }, [title, price, city, state, imagesFiles])

    const imagekitUpload = (file: File): Promise<{ url: string, previewUrl: string }> => {
        return new Promise((resolve, reject) => {
            imagekit.upload({
                ...
                err ? reject('Unable to upload image') : resolve({ url: result.url, previewUrl: result.thumbnailUrl })
            })
        })
    }

    const uploadImages = async (files: File[]): Promise<Array<{ url: string, previewUrl: string }>> => {
                ...
                <div>
                    <div className=''>
                        <TextInput
                            ...
                            placeholder='Title'
                        />
                        <ErrorMessage text='Title should be less than 65 characters' show={title?.length > 65} />
                        <ErrorMessage text='Title should be specified' show={title?.length < 1} />
                    </div>
                    <div className='flex flex-col md:flex-row mt-5'>
                        <div className=' md:pr-2' style={{ flex: 2 }}>
                            <TextInput
                                ...
                                placeholder='Price'
                            />
                            <ErrorMessage text='Price should be specified' show={!price || !(parseFloat(price) > 0)} />
                        </div>
                    ...
                    <div className='flex flex-col md:flex-row mt-5'>
                        <div className='md:pr-2' style={{ flex: 1 }}>
                            <TextInput
                                ...
                                placeholder='City'
                            />
                            <ErrorMessage text='City should be specified' show={city?.length < 1} />
                    ...
                    <div className='mt-5'>
                        ...
                        />
                        <ErrorMessage text='Description should be specified' show={description?.length < 1} />
                    </div>
                    <div className='mt-5'>
                        ...
                        />
                        <ErrorMessage text='Images should be selected' show={imagesFiles?.length < 1} />
                    </div>
